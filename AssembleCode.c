...
void checkCountdown(unsigned long *lastTimeAlert, int *repeatTime, int slotPin, int slotPinM, bool *taketime, bool *boxIsOpen, String MedName) {
  currentTime = millis();
  if (currentTime - *lastTimeAlert >= 300000) {  // 5 minutes delay
...
void taken() {
  if (digitalRead(reedPin) == HIGH && taketime1 == true) {
    boxIsOpen1 = true;
    Serial.println("is in taketime1");
  }
  if (digitalRead(reedPin) == HIGH && taketime2 == true) {
    boxIsOpen2 = true;
    Serial.println("is in taketime2");
  }
  if (digitalRead(reedPin) == HIGH && taketime3 == true) {
    boxIsOpen3 = true;
...
void Med_log(String med, bool taken, String time) {
  ...
  if (med == FirstMedName) {
    ...
    if (taken == true) {
      ...
      writeFile("/log.txt", FirstMedName + " missed at " + date + " " + time);
    }
    ...
    if (taken == true) {
      ...
      writeFile("/log.txt", SecondMedName + " missed at " + date + " " + time);
    }
    ...
    if (taken == true) {
      ...
      writeFile("/log.txt", ThirdMedName + " missed at " + date + " " + time);
    }
  }
}
